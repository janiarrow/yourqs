...
// Include the main TCPDF library (search for installation path).
	...
	if (isset($_GET) & !empty($_GET['pid'])) {
		...
		$delSql = "DELETE FROM project WHERE project_id=$pid";
		
		$res = mysqli_query($connection, $delSql);

		if ($res) {
			$fmsg = "Project deleted successfully";
		} else {
			$fmsg = "Failed to delete the project";
		}
		
	}else if (isset($_GET) & !empty($_GET['subpid'])) {
...
// ---------------------------------------------------------
		...
		while($rooms = mysqli_fetch_assoc($roomRS)){
			...
			$tbl = "<br /><br /><div style=\"color:#2b6ca3;\">
				<table style=\"border:2px solid #e87511;\" cellpadding=\"5\">
			<tr color=\"black\" bgcolor=\"#EA8228\"><td colspan=\"3\"><b>Room Details</b></td></tr>";
...
//		echo file_exists('pdf/'.$project_name.'_'.$uid.'_'.$id.'.pdf');
		...
		if (file_exists('pdf/'.$project_name.'_'.$uid.'_'.$id.'.pdf')) {
			...
			$mail->Username = 'site@bisco.co.nz';                 //yib00002bn@aspire2student.ac.nz SMTP username
			$mail->Password = 'aspire218';//'0778027173a';                           // SMTP password
			$mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
			$mail->Port = 587;                                    // TCP port to connect to

			$mail->From='site@bisco.co.nz';//vjarosha@gmail.com';
	...
	$query   = "SELECT * FROM project WHERE user_id=$usrid AND status ='pending' order by date ASC ";
...
<!--===============================================================================================-->
	...
	<body>
		...
		<div class="topnav" id="myTopnav">
			<a class="active">Project List</a>
			<a href="submitted.php">Submitted Projects</a>
				...
				<form action="project.php" method="post">
					<div class="modal-body">						
						<span class="form-group has-float-label">
							<input class="form-control" type="text" id="projname" name="projname1" placeholder=" "/>
							<label>Project Name</label>
						  </span>
<!--
						<div class="wrap-input100 rs1 validate-input" data-validate = "Project Name is required">
							<input class="input100" type="text" name="projname">
				  ...
				  <tbody>
					  ...
					  <td scope="row"><a href="project.php?id=<?php echo $project['project_id']; ?>"> 
					    <?php echo $project['project_name']." <br />".$project['date']; ?> </a></td>
						   ...
						   <td scope="row"> <small><a onclick="submitpj('<?php echo $project['project_id']; ?>')" class="btn btn-info btn-block btn-md" >Submit</a></small></td>
					  <td  scope="row"><a onclick="delclick('<?php echo $project['project_id']; ?>')"  class="btn btn-sm"> <img width="40" height="30" src="images/icons/Recycle_Bin_Full.png"></a></td>
			...
			swal({
				...
				.then((value) => {
					...
					case "remove":
						...
						window.location="home.php?pid="+pid;
						  break;
			...
			swal({
				...
				.then((value) => {
					...
					case "submit":
						...
						window.location="home.php?subpid="+pid;
						  break;
